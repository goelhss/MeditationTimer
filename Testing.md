...
- `TEST_SUITE=always`: package, manifest, timer/preparation/reminder math, preset and streak/statistics/vacation-pause rules, independent streak-control policy, cue-mode and synthesized-sound policy, completion policy, core log/resolution persistence-format tests, merge-with-dedup restore, and backup security-policy checks. Required before every build and push.
...
- `TEST_SUITE=ui`: required tabs, lotus launch/completion screens, preparation and sound/timer-display controls, seconds-visible digital/analog timer wiring, live chime/vibration/dim switching, streak counting/reminder switches, vacation pause UX, reminder scheduling/notification wiring, About/version history, What's New, dim-screen, and diagnostics source policies.
